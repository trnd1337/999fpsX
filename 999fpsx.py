...
def ensure_default_profiles_copied():
    try:
...
class WindowsFallbackBackend(BackendBase):
    ...
    def apply_profile(self, profile):
        ...
        run_cmd(["powercfg", "/setacvalueindex", "SCHEME_CURRENT", "SUB_PROCESSOR", "PROCTHROTTLEMAX", str(profile.get("max_processor_state_ac", 100))])
        if "max_processor_state_dc" in profile:
            run_cmd(["powercfg", "/setdcvalueindex", "SCHEME_CURRENT", "SUB_PROCESSOR", "PROCTHROTTLEMAX", str(profile.get("max_processor_state_dc", 100))])
        run_cmd(["powercfg", "/S", "SCHEME_CURRENT"])
        if profile.get("disable_core_parking"):
            run_cmd(["powercfg", "/setacvalueindex", "SCHEME_CURRENT", "SUB_PROCESSOR", "CPUPARKINGMAXCORES", "100"])
            run_cmd(["powercfg", "/setacvalueindex", "SCHEME_CURRENT", "SUB_PROCESSOR", "CPUPARKINGMINCORES", "0"])
            run_cmd(["powercfg", "/S", "SCHEME_CURRENT"])
...
class AmdBackend(BackendBase):
    ...
    def read_telemetry(self):
        freq = psutil.cpu_freq()
        return {"freq": freq.current if freq else None, "load": psutil.cpu_percent(interval=None), "temp": read_cpu_temp()}
    def apply_profile(self, profile):
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        if AUTO_ELEVATE and not is_admin():
            relaunch_as_admin()
            QtWidgets.QMessageBox.information(None, "Elevation", "Relaunching with administrator privileges. Please re-run the app if needed.")
            sys.exit(0)
        ...
        if self.vendor == "AMD":
            ...
            self.backend = IntelBackend()
        else:
            self.backend = WindowsFallbackBackend()

        self.pmgr = ProfileManager()
        self.telemetry_thread = TelemetryThread(self.backend, interval=self.settings["telemetry_interval"])
    ...
    def init_ui(self):
        ...
        # Right: settings + vendor + power plan
        ...
        self.thermal_spin.valueChanged.connect(lambda v: self.on_setting_change("thermal_cutoff", float(v)))
        ...
        self.max_ac_spin.valueChanged.connect(lambda v: self.on_setting_change("max_processor_state_ac", int(v)))
        ...
        self.max_dc_spin.valueChanged.connect(lambda v: self.on_setting_change("max_processor_state_dc", int(v)))
        ...
        self.park_chk.stateChanged.connect(lambda s: self.on_setting_change("disable_core_parking", bool(s)))
        ...
        self.telemetry_spin.valueChanged.connect(lambda v: self.on_setting_change("telemetry_interval", float(v)))
        ...
        self.pl1_spin.valueChanged.connect(lambda v: self.on_setting_change("pl1_watts", int(v)))
        ...
        self.pl2_spin.valueChanged.connect(lambda v: self.on_setting_change("pl2_watts", int(v)))
        s_layout.addRow("PL2 (Watts)", self.pl2_spin)

        self.pbo_chk = QtWidgets.QCheckBox("Attempt PBO enable via vendor CLI (if available)")
        self.pbo_chk.setChecked(self.settings.get("attempt_pbo", False))
        self.pbo_chk.stateChanged.connect(lambda s: self.on_setting_change("attempt_pbo", bool(s)))
        ...
        self.vendor_cli_edit = QtWidgets.QLineEdit(self.settings.get("vendor_cli_path", ""))
        self.vendor_cli_edit.setPlaceholderText("Path to vendor CLI (AMDRyzenMaster/AMDRMCLI)")
        v_layout.addRow("Vendor CLI", self.vendor_cli_edit)

        vendor_browse = QtWidgets.QPushButton("Browse")
        def _browse_vendor():
            fn, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select vendor CLI executable", str(Path.cwd()), "Executables (*.exe);;All files (*)")
            if fn:
        ...
        self.vendor_profile_edit = QtWidgets.QLineEdit(self.settings.get("vendor_profile_name", ""))
        self.vendor_profile_edit.setPlaceholderText("Vendor profile name (if applicable)")
        v_layout.addRow("Vendor profile", self.vendor_profile_edit)

        self.vendor_template_edit = QtWidgets.QLineEdit(self.settings.get("vendor_command_template", ""))
        self.vendor_template_edit.setToolTip("Template: {cli} and {profile} will be replaced. Example: {cli} -a ApplyProfile \"{profile}\"")
        ...
        self.auto_import_chk.stateChanged.connect(lambda s: self.on_setting_change("auto_import_plan", bool(s)))
        ag_layout.addWidget(self.auto_import_chk)

        self.auto_activate_chk = QtWidgets.QCheckBox("Activate after import")
        self.auto_activate_chk.setChecked(self.settings.get("auto_import_activate", False))
        self.auto_activate_chk.stateChanged.connect(lambda s: self.on_setting_change("auto_import_activate", bool(s)))
        ag_layout.addWidget(self.auto_activate_chk)

        self.auto_path_edit = QtWidgets.QLineEdit(self.settings.get("auto_import_path", ""))
        self.auto_path_edit.setPlaceholderText("Path to .pow file")
        self.auto_path_edit.editingFinished.connect(lambda: self.on_setting_change("auto_import_path", self.auto_path_edit.text().strip()))
        ag_layout.addWidget(self.auto_path_edit)

        pick_btn = QtWidgets.QPushButton("Browse")
        def _pick_pow():
            fn, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select power plan file", str(USER_PROFILES_DIR), "Power plan (*.pow);;All files (*)")
            if fn:
        ...
        import_btn.clicked.connect(lambda: self._manual_import_now())
        ...
        self.helper_label = QtWidgets.QLabel("Preview shows the commands that will run. Apply/Import/Vendor require Admin.")
    ...
    def on_telemetry(self, data):
        ...
        if self.boost_active and temp and temp >= self.settings.get("thermal_cutoff", self.thermal_limit):
            self.append_log(f"Thermal limit reached {temp:.1f} °C. Reverting.")
            self.revert()

    def _gather_profile_from_settings(self):
        p = {
            "thermal_cutoff": float(self.settings.get("thermal_cutoff", self.thermal_limit)),
            "disable_core_parking": bool(self.settings.get("disable_core_parking", False)),
            "max_processor_state_ac": int(self.settings.get("max_processor_state_ac", 100)),
            "max_processor_state_dc": int(self.settings.get("max_processor_state_dc", 100)),
            "telemetry_interval": float(self.settings.get("telemetry_interval", 0.8)),
            "affinity_mode": self.settings.get("affinity_mode", "all"),
            "custom_affinity_mask": int(self.settings.get("custom_affinity_mask", 0)),
            "vendor_profile_name": self.vendor_profile_edit.text().strip()
        }
        return p

    def _build_preview_text(self, profile):
        lines = []
        lines.append("The following actions will be performed:")
        lines.append(f"- Set active power plan to High Performance (GUID {WindowsFallbackBackend.HIGH_GUID})")
        lines.append(f"- Set max processor state (AC) to {profile.get('max_processor_state_ac', 100)}%")
        lines.append(f"- Set max processor state (DC) to {profile.get('max_processor_state_dc', 100)}%")
        if profile.get("disable_core_parking"):
        ...
        if affinity == "custom":
            lines.append(f"  - Custom affinity mask: 0x{profile.get('custom_affinity_mask',0):X}")
        lines.append(f"- Thermal cutoff: {profile.get('thermal_cutoff')} °C (auto-revert if exceeded)")
        lines.append(f"- Rollback window: {self.rollback_seconds} seconds (auto revert unless confirmed)")
        if self.settings.get("pl1_watts"):
            lines.append(f"- PL1 target: {self.settings.get('pl1_watts')} W (applied via vendor CLI if supported)")
        if self.settings.get("pl2_watts"):
            lines.append(f"- PL2 target: {self.settings.get('pl2_watts')} W (applied via vendor CLI if supported)")
        if self.settings.get("attempt_pbo"):
            lines.append("- Attempt to enable PBO via vendor CLI (if available)")
        return "\n".join(lines)
    ...
    def apply_settings_now(self):
        if not is_admin():
            ...
            QtWidgets.QMessageBox.warning(self, "Admin required", "Run as Administrator to apply system settings")
            return
    ...
    def _apply_affinity(self, profile):
        ...
        try:
            ...
            if mode == "all":
                ...
                if cores:
                    ...
                    self.append_log("Custom affinity mask produced no cores; no change")
        except Exception as e:
        ...
        for line in text.splitlines():
            ...
            if line.lower().startswith("power scheme guid:"):
                try:
                    ...
                    guid = parts.split()[0]
                    name_part = line[line.find("(")+1:line.rfind(")")] if "(" in line and ")" in line else ""
    ...
    def rename_selected_plan(self):
        if not is_admin():
            QtWidgets.QMessageBox.warning(self, "Admin required", "Run the app as Administrator to rename power plans")
        ...
        if not new_name:
            ...
            return
        reply = QtWidgets.QMessageBox.question(self, "Confirm rename",
        ...
        if code == 0:
            ...
            QtWidgets.QMessageBox.information(self, "Success", "Power plan renamed successfully")
            self.refresh_power_plans_ui()
        else:
            self.append_log(f"Rename failed: {err or out}")
            QtWidgets.QMessageBox.critical(self, "Error", f"Rename failed: {err or out}")
    ...
    def import_powerplan_file(self, pow_path: str, set_active: bool = False, guid: str | None = None):
        if not is_admin():
    ...
    def _manual_import_now(self):
        ...
        if not path:
            ...
            return
        if not is_admin():
            QtWidgets.QMessageBox.warning(self, "Admin required", "Run as Administrator to import power plans")
            self.append_log("Manual import aborted: admin required")
            return
        ok, msg = self.import_powerplan_file(path, set_active=self.auto_activate_chk.isChecked(), guid=self.settings.get("auto_import_guid"))
        ...
        try:
            ...
            if auto and path:
                ...
                if not p.exists():
                    ...
                    if candidate.exists():
                        path = str(candidate)
                self.append_log(f"Attempting auto-import of {path} (activate={activate})")
                ok, msg = self.import_powerplan_file(path, set_active=activate, guid=guid)
                self.append_log(f"Auto-import: {ok} {msg}")
                if not ok and "Administrator required" in msg:
                    QtWidgets.QMessageBox.warning(self, "Admin required", "Auto-import requires Administrator privileges. Relaunch the app as Administrator to import and activate the plan.")
        except Exception as e:
    ...
    def _detect_vendor_cli(self):
        ...
        if candidates:
            ...
            QtWidgets.QMessageBox.information(self, "Detected", f"Detected AMD CLI: {chosen}")
        else:
            QtWidgets.QMessageBox.information(self, "Not found", "No AMD CLI found in common locations. Please browse to the vendor CLI executable.")
    ...
    def _apply_vendor_profile(self):
        ...
        if not cli:
            ...
            QtWidgets.QMessageBox.warning(self, "Vendor CLI", "Set vendor CLI path first")
            return
        ...
        try:
            if template and "{cli}" in template:
                ...
                args = [cli, "-a", "ApplyProfile", profile] if profile else [cli, "-a", "ApplyProfile"]
                code, out, err = run_cmd(args)
            if code == 0:
                self.append_log(f"Vendor profile applied: {profile} (cli={cli})")
                QtWidgets.QMessageBox.information(self, "Vendor", f"Vendor profile applied: {profile}")
            else:
                self.append_log(f"Vendor apply failed: {err or out}")
                QtWidgets.QMessageBox.critical(self, "Vendor error", f"Apply failed: {err or out}")
        except Exception as e:
            self.append_log(f"Vendor apply exception: {e}")
            QtWidgets.QMessageBox.critical(self, "Vendor error", f"Exception: {e}")
    ...
    def _apply_vendor_power_limits(self):
        cli = self.vendor_cli_edit.text().strip()
        if not cli or not Path(cli).exists():
            self.append_log("Vendor CLI not set; skipping vendor power limit application")
            return
    ...
    def save_profile(self):
        ...
        if self.include_powerplan_chk.isChecked():
            ...
            if ok:
                self.append_log(f"Exported power plan with profile: {pow_out.name}")
            else:
    ...
    def load_profile(self):
        ...
        self.thermal_spin.setValue(int(self.settings.get("thermal_cutoff", self.thermal_limit)))
        self.park_chk.setChecked(self.settings.get("disable_core_parking", False))
        self.max_ac_spin.setValue(int(self.settings.get("max_processor_state_ac", 100)))
        self.max_dc_spin.setValue(int(self.settings.get("max_processor_state_dc", 100)))
        self.telemetry_spin.setValue(float(self.settings.get("telemetry_interval", 0.8)))
